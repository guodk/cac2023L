Require Export MK_Definitions_Axioms.
...
Proof.
  intros; eqext; deHun; deGun; auto; [right|right|left|left]; deGun; auto.
Qed.
...
Proof. unfold Included; intros. apply H in H0. appA2H H0; tauto. Qed.
...
Theorem MKT42' : ∀ x, Ensemble ([x]) -> Ensemble x.
Proof.
  intros. Absurd. apply MKT43 in H0. elim MKT39. rewrite <- H0; auto.
Qed.
...
Proof.
  intros. apply MKT43 in H. rewrite H; split; symmetry;
  [apply MKT34|apply MKT34'].
Qed.

(* 并的公理IV 如果x是一个集同时y是一个集，则x∪y也是一个集*)

Corollary AxiomIV': ∀ x y,
...
Theorem MKT46a : ∀ {x y}, Ensemble x -> Ensemble y -> Ensemble ([x|y]).
Proof.
...
Theorem MKT47b : ∀ x y, Ensemble x -> Ensemble y -> ∪[x|y] = x ∪ y.
Proof.
  intros; unfold Unordered; eqext; appA2H H1; appA2G.
  - rdeHex. deHun; eins H3.
  - destruct H2; [exists x|exists y]; split; auto; apply MKT46b; auto. 
Qed.
...
Proof.
  ...
  - apply AxiomIV' in H as []. apply MKT42', AxiomIV' in H0 as [].
    split; apply MKT42'; auto.
  - apply AxiomIV; apply MKT42; [|apply AxiomIV]; apply MKT42; auto.
Qed. *)
...
Proof.
  intros. apply AxiomIV' in H as []. apply MKT42', AxiomIV' in H0 as [].
  split; apply MKT42'; auto.
Qed.
...
Proof. unfold Included; intros. deGun; auto. Qed.
...
Proof.
  repeat split; intros; repeat rewrite Lemma50a; repeat rewrite Lemma50b; auto; 
  [apply MKT44|apply MKT44|apply MKT47b|apply MKT47a]; auto.
Qed.

Lemma Lemma50' : ∀ (x y: Class),
...
Proof.
  ...
  [apply MKT24'|apply MKT24|
  rewrite <- MKT34'|rewrite MKT34]; auto.
Qed.
...
Theorem MKT54a : ∀ x y,
  Ensemble x -> Ensemble y -> First ([x,y]) = x.
Proof.
  intros; unfold First. apply MKT50; auto.
Qed.

Theorem MKT54b : ∀ x y,
  Ensemble x -> Ensemble y -> Second ([x,y]) = y.
Proof.
...
Theorem MKT55 : ∀ x y u v,
  Ensemble x -> Ensemble y -> ([x,y] = [u,v] <-> x = u /\ y = v).
Proof.
  split; intros; [|destruct H1; subst; auto].
  assert (Ensemble ([x,y])); auto. rewrite H1 in H2. apply MKT49b in H2 as [].
  rewrite <- (MKT54a x y), H1, <- (MKT54b x y), H1, 
    MKT54a, MKT54b; auto.
Qed.
...
Proof.
  ...
  - New H0. apply H in H0 as [? [?]]; subst. appoA2G. apply invp1; auto.
Qed.
...
Proof. split; [red|]; intros; [eins H1|pins H1; pins H2]. Qed.

(* 定理64 如果f是一个函数同时g是一个函数，则 f∘g 也是一个函数 *)

Fact PisRel : ∀ P, Relation \{\ P \}\.
Proof. unfold Relation; intros. PP H a b; eauto. Qed.
...
Fact dos1 : ∀ {f x} y, Function f ->
  [x,y] ∈ f -> dom(f ~ [[x,y]]) = dom(f) ~ [x].
Proof.
...
Fact ros1 : ∀ {f x y}, Function f⁻¹ ->
  [x,y] ∈ f -> ran(f ~ [[x,y]]) = ran(f) ~ [y].
Proof.
  intros. eqext; appA2H H1; destruct H2.
...
Corollary Property_Fun : ∀ y f x,
  Function f -> [x,y] ∈ f -> y = f[x].
Proof.
...
Proof.
  ...
  - appA2G; intros. appA2H H3. pinfus H4. elim H. eapply Property_dom; eauto.
Qed.

Fact Einr : ∀ {f z}, Function f -> z ∈ ran(f) -> ∃ x, x ∈ dom(f) /\ z = f[x].
Proof.
...
Fact subval : ∀ {f g}, 
  f ⊂ g -> Function f -> Function g -> ∀ u, u ∈ dom(f) -> f[u] = g[u].
Proof.
...
Theorem MKT71 : ∀ f g,
  Function f -> Function g -> (f = g <-> ∀ x, f[x] = g[x]).
Proof.
...
Lemma Ex_Lemma74 : ∀ {x y}, Ensemble x -> Ensemble y ->
  ...
  Function f /\ dom(f) = x /\ ran(f) = \{ λ z, ∃ u, u ∈ x /\ z = [u] × y \}.
Proof.
  repeat split; intros; auto.
  - appoA2H H1; appoA2H H2. deand. subst. auto.
  - eqext.
...
Proof.
  ...
  - red; intros. apply MKT38b; [apply MKT74; auto|].
    red; intros. appA2H H1. deand. New H2. apply H3 in H2 as [? []]. subst.
    New (Property_dom H6). New (Property_ran H6). appoA2G.
Qed.
...
Proof.
  ...
  - assert ([u] ∪ [v] ∪ [w] ⊂ x). { red; intros. deHun; eins H8. }
    assert ([u] ∪ [v] ∪ [w] ≠ Φ). { apply NEexE; exists u. deGun. eauto. }
...
Proof.
  ...
  - appA2G. split; auto. destruct H4. New (H4 _ _ H1 (H _ H6)). deor; auto.
...
Theorem MKT92 : ∀ {x y z r},
  rSection x r z -> rSection y r z -> x ⊂ y \/ y ⊂ x.
Proof.
  intros. TF (x ⊂ y); auto.
...
Theorem MKT94 : ∀ {x r y f},
  rSection x r y -> Order_Pr f r r -> On f x -> To f y -> 
  (∀ u, u ∈ x -> ~ Rrelation f[u] r u).
Proof.
  intros; intro. destruct H, H5, H5, H0, H8, H9, H1, H2 as [_].
  assert (u ∈ \{ λ u, u ∈ x /\ Rrelation f[u] r u \} ). { appA2G. }
  assert (∃ z, FirstMember z r \{ λ u, u ∈ x /\ Rrelation f[u] r u \}).
...
Lemma f11vi : ∀ f u,
  Function f -> Function f⁻¹ -> u ∈ ran(f) -> f[(f⁻¹)[u]] = u.
Proof.
...
Lemma f11iv : ∀ f u,
  Function f -> Function f⁻¹ -> u ∈ dom(f) -> (f⁻¹)[f[u]] = u.
Proof.
  intros. apply Property_Value, invp1, Property_Fun in H1; auto.
Qed.

Fact f11pa : ∀ {f x y}, 
  Function1_1 f -> [x,y] ∈ f -> Function1_1 (f ~ [[x,y]]).
Proof.
  intros * [] ?. repeat split; try red; intros.
  - appA2H H2. apply H; tauto.
  - apply setminp in H2. apply setminp in H3. deand. eapply H; eauto.
  - PP H2 a b; eauto.
  - appoA2H H2. appoA2H H3. apply setminp in H4. apply setminp in H5.
    deand. eapply H0; apply invp1; eauto. 
Qed.
...
Proof.
  ...
  - apply fupf; auto.
  - rewrite reqdi in H3. rewrite uiv. rewrite siv; auto. apply fupf; auto.
Qed.
...
Proof.
  intros. destruct H as [? [? []]]. split; auto; split; try red; intros.
  - PP H3 a b. eauto.
  - apply ->invp1 in H3. apply -> invp1 in H4. New H3. New H4.
    apply Property_Fun in H3; apply Property_Fun in H4; subst; auto.
    New (Property_dom H5). New (Property_dom H6). New (Property_ran H6).
    destruct H0. New (H0 _ _ H3 H7). deor; auto.
...
Proof.
  ...
  New (Property_dom H21). apply Property_Fun in H21; auto. rewrite H21 in H6.
  ...
  destruct H1 as [? []], H2 as [? []]. New (H25 _ _ (H2 _ H22) H19 H23).
  apply (H14 v); auto. appA2G. split; deGin; auto. 
  intro. rewrite <- H21 in H29. destruct G as [? _]. 
  eapply f11inj in H29; eauto. subst.
  eapply Property_Asy; eauto. apply MKT88a; auto.
Qed.

Lemma le97 : ∀ f g, Function f -> Function g -> 
  (∀ a, a ∈ (dom(f) ∩ dom(g)) -> f[a] = g[a]) -> dom( f) ⊂ dom( g) -> f ⊂ g.
Proof.
...
Proof.
  ...
  - assert (\{ λ a, a ∈ (dom(f) ∩ dom(g)) /\ f[a] <> g[a] \} <> Φ).
    { intro. apply H5; intros. Absurd. feine a. rewrite <- H6. appA2G. }
    assert (\{ λ a, a ∈ (dom( f) ∩ dom( g)) /\ f [a] <> g [a] \} ⊂ dom(f)).
    { red; intros. appA2H H7. deand. deHin; auto. }
    inversion H1 as [? [[_ ?] _]]. destruct (H9 _ (MKT28 H7 H8) H6) as [u]. 
...
Lemma Lemma99c : ∀ y r x a b,  
  rSection y r x -> a ∈ y -> ~ b ∈ y -> b ∈ x -> Rrelation a r b.
Proof.
...
Theorem MKT99 : ∀ {r s x y},
  WellOrdered r x -> WellOrdered s y ->
  ∃ f, Function f /\ Order_PXY f x y r s /\(dom(f) = x \/ ran(f) = y).
Proof.
  intros. set (f:=\{\ λ u v, u ∈ x /\ 
  (∃ g, Function g /\ Order_PXY g x y r s /\ u ∈ dom(g) /\ [u,v] ∈ g ) \}\).
  assert (Function f).
  { split; intros; unfold f; auto. appoA2H H1. appoA2H H2. deand. rdeHex.
    RN x2 g1. RN x1 g2. red in H7, H10. deand.
  ...
  { split; [|split]; try red; intros; auto.
    ...
    - appA2H H3. rdeHex. appoA2H H5. deand. rdeHex. RN x1 g.
      inversion H8. deand. destruct H14, H16. New (H17 _ _ H2 H9 H4).
      appA2G. exists g[u]. New H18. New H18.
  ...
  { split; [|split]; try red; intros; auto.
    ...
    - appA2H H4. rdeHex. appoA2H H6. deand. rdeHex. RN x1 g.
      ...
      apply AxiomII in H19 as [? [z]]. New H20. apply Property_dom in H20.
      New H20. apply Property_Value in H20; auto.
  ...
  { red; deandG; auto. destruct H2 as [? []], H3 as [? []]. 
    ...
    - New (subdom H25). rewrite (subval H25); auto. apply H15; auto. }
...
Theorem MKT100' : ∀ r s x y,
  WellOrdered r x /\ WellOrdered s y -> Ensemble x -> ~ Ensemble y ->
  ∀ f, Function f /\ Order_PXY f x y r s /\ dom(f) = x ->
  ∀ g, Function g /\ Order_PXY g x y r s /\ dom(g) = x -> f = g.
Proof.
...
Proof.
  ...
  apply AxiomVII in H0 as [? []]. eins H0. feine x. rewrite <- H1. appA2G.
Qed.
...
Proof.
  intros. assert (x ∈ [x|y]) by appA2G. assert (y ∈ [x|y]) by appA2G.
...
Proof.
  intro. assert (E ∈ [E]) by appA2G. assert ([E] ∈ [E, [E]]) by appA2G.
...
Proof.
  ...
  { red; intros. deandG; auto. intros. appoA2H H6. eapply H2; eauto. } 
...
Proof.
  ...
  - destruct H, H0. unfold Full, Included in *; intros. deHin. deGin; eauto.
Qed.
...
Corollary Th110ano : ∀ {x y}, Ordinal x -> Ordinal y -> x ∈ y \/ y ⊂ x.
Proof.
...
Proof.
  intros. New (MKT110 H1 H2). deor; auto; [left|right;left]; appoA2G.
Qed.
...
Proof.
  ...
  - appA2H H0. appA2H H1. rdeHex. New (H _ H4). New (H _ H5).
    appA2H H6. appA2H H7. eapply MKT111 in H2; eapply MKT111 in H3; eauto.
    apply Lemma113; auto.
...
Proof.
  intros. New H. New (MKT107 MKT113a). apply H2 in H as [? []]; auto.
  assert (∩x = x0).
...
Theorem MKT126c : ∀ f x,
  Function f -> (∀ y, y ∈ dom(f|(x)) -> (f|(x)) [y] = f [y]).
Proof.
  intros. apply subval; auto; [|apply MKT126a]; auto.
  red; intros. appA2H H1. tauto.
Qed.

Corollary frebig : ∀ f x,
  Function f -> dom(f) ⊂ x -> f|(x) = f.
Proof.
  intros; eqext.
  - appA2H H1; tauto.
  - appA2G; split; auto. New H1. apply H in H1 as [? [?]]. subst. New H2.
    apply Property_dom in H2. appoA2G; split; auto. apply MKT19; ope.
Qed.

Corollary fresub : ∀ f h,
...
Proof.
  ...
  - assert (∃ u, FirstMember u E \{λ a, a ∈ dom(f)∩dom(h) /\ f[a]<>h[a]\}).
...
Proof.
  intros. set (f:=\{\ λ u v, u ∈ R /\ (∃ h, Function h /\ Ordinal dom(h) /\ 
  (∀ z, z ∈ dom(h) -> h[z] = g [h|(z)] ) /\ [u,v] ∈ h ) \}\).
  assert (Function f).
  { split; [unfold f; auto|]; intros. appoA2H H. appoA2H H0.
    destruct H1 as [? [h1]], H2 as [? [h2]]. deand.
    destruct (MKT127 H3 H8 H9 H4 H5 H6); [eapply H3|eapply H4]; eauto. }
  ...
  { apply MKT114; unfold rSection; deandG; intros.
    ...
    - appA2H H1. destruct H3. appoA2H H3. destruct H4 as [? [h]]. deand.
      apply Property_dom in H8. appoA2H H2. eapply H6 in H9; eauto.
      apply Property_Value in H9; auto.
      appA2G. exists h[u]. appoA2G. split; eauto. }
  assert (K1: ∀ x, x ∈ dom(f) -> f[x] = g[f|(x)]); intros.
  { appA2H H1. destruct H2. appoA2H H2. destruct H3 as [? [h]]. deand.
    ...
    apply Property_dom in H7. rewrite <- (subval H8), H6; auto. f_equal.
      ...
      { apply MKT114; unfold rSection, h; deandG; intros.
        ...
        - rewrite undom in H11|-*. deGun. deHun.
          ...
          + rewrite domor in H11; auto. eins H11.
            appoA2H H12. left. Absurd. destruct (H5 u); auto. appoA2G. }
...
Lemma lem128 : ∀ {f g h},
  ...
  Ordinal dom(h)-> (∀ x, Ordinal_Number x -> f[x] = g [f|(x)]) ->
  (∀ x, Ordinal_Number x -> h [x] = g [h|(x)]) -> h ⊂ f -> h = f.
Proof.
...
Fact EnEm : Ensemble Φ. Proof. destruct AxiomVIII as [? [? []]]. eauto. Qed.
...
Theorem MKT134 : ∀ {x}, x ∈ W -> (PlusOne x) ∈ W.
Proof.
  intros. appA2H H. destruct H0. appA2G; [|split].
  - apply AxiomIV; auto.
  -  assert (x ∈ R). { appA2G. } apply Lem123 in H2. appA2H H2; auto.
...
Theorem MKT135 : Φ ∈ W /\ (∀ x, x ∈ W -> Φ ≠ PlusOne x).
Proof.
  split; intros.
...
Theorem MKT135b : ∀ x, x ∈ W -> Φ ≠ PlusOne x.
Proof.
...
Theorem MKT137 : ∀ x,
  x ⊂ W -> Φ ∈ x -> (∀ u, u ∈ x -> (PlusOne u) ∈ x) -> x = W.
Proof.
  intros. Absurd.
  assert (∃ y, FirstMember y E (W ~ x)).
  { apply (@MKT107 W); auto; red; intros.
    - appA2H H3; tauto.
    - apply H2. eqext; auto. Absurd. feine z. rewrite <- H3; auto. }
  destruct H3 as [y []]. appA2H H3. destruct H5. appA2H H6. appA2H H5.
...
Theorem MiniMember_Principle : ∀ S,
  S ⊂ W -> S ≠ Φ -> ∃ a, a ∈ S /\ (∀ c, c ∈ S -> a ≼ c).
Proof.
...
Definition En_S (P :Class -> Prop) := \{ λ x, x ∈ W /\ ~ (P x) \}.

Theorem Mathematical_Induction : ∀ (P0 :Class -> Prop),
  P0 Φ -> (∀ k, k ∈ W -> P0 k -> P0 (PlusOne k)) ->
  (∀ n, n ∈ W -> P0 n).
Proof.
  intros. assert (\{λ x, x ∈ W /\ P0 x \} = W).
...
Fact c2fp : ∀ {x c g P f}, Ensemble x -> dom(c) = μ ~ [Φ] -> 
  (∀ x, x ∈ dom(c) -> c[x] ∈ x) -> (∀ h, Ensemble h -> g[h] = c[P h]) ->
  (∀ x, Ordinal_Number x -> f[x] = g[f|(x)]) ->
   Function f -> Ordinal dom(f) -> 
  (∀ u, u ∈ dom(f) -> Ensemble (P (f|(u))) -> f[u] ∈ (P (f|(u)))).
Proof.
  intros. assert (Ordinal_Number u). { appA2G. eapply MKT111; eauto. }
  assert (Ensemble (f|(u))).
...
Theorem MKT140 : ∀ x,
  Ensemble x -> ∃ f, Function1_1 f /\ ran(f) = x /\ Ordinal_Number dom(f).
Proof.
  ...
  { split; auto; split; try red; intros.
    ...
    - appoA2H H8. appoA2H H9. New H10. New H11.
      ...
      + elim H22; appA2G. exists y. appA2G; rxo. rewrite H11 in H10.
        split; auto. appoA2G.
      + elim H18; appA2G. exists z. appA2G; rxo. split; auto. appoA2G. }
  assert (ran(f) ⊂ x).
...
Proof.
  ...
  { intros. apply (c2fp H H2 H1 H3 H6 H4 H5) in H7; [appA2H H7; tauto|auto]. }
  assert (∀ u v, u ∈ dom(f) -> v ∈ dom(f) -> u ∈ v -> 
    f[u] ⊂ f[v] /\ f[u] <> f[v]); intros.
  { apply (c2fp H H2 H1 H3 H6 H4 H5) in H9; auto. appA2H H9. deand.
    apply H13. apply Property_Value in H8; auto. appA2G; ope. exists u.
    appA2G. split; auto. appoA2G; split; auto. apply MKT19; ope. }
  exists (∪ran(f)). deandG; try red; intros.
  - appA2H H9. appA2H H10. rdeHex.
    appA2H H13. appA2H H14. destruct H15, H16. New H15. New H16.
    apply Property_Fun in H15; apply Property_Fun in H16; subst; auto.
...
Theorem MKT147 : ∀ y x z,
  x ≈ y -> y ≈ z -> x ≈ z.
Proof.
  intros. destruct H as [f1], H0 as [f2]. deand. exists (f2∘f1). deandG; subst.
...
Proof.
  ...
  - apply (wosub R E y); auto; [apply (MKT107 MKT113a)|].
    red; intros. apply H in H1. appA2H H1. destruct H2. appA2H H2. appA2G.
Qed.
...
Proof.
  ...
  { apply (wosub R E S); auto; [apply (MKT107 MKT113a)|..].
    ...
    - appA2H H2. apply NEexE. exists dom(f). appA2G. unfold Equivalent; eauto. }
...
Proof.
  ...
  assert (WellOrdered E x). { eapply wosub; eauto. apply MKT107, H1. }
  ...
  assert (ran(f) ≼ y). 
  { red. TF (ran(f) = y); auto. left. eapply MKT108; auto. apply H11. }
  appA2H H5. destruct H18. appA2H H18.    
  assert (dom(f) ≈ ran(f)). { red; exists f; split; split; auto. }
...
Theorem MKT160 : ∀ {f},
  Function f -> Ensemble f -> P[ran(f)] ≼ P[dom(f)].
Proof.
  intros. destruct AxiomIX as [c [[]]].
  ...
  assert (dom(g) ≈ ran(g)).
  { exists g; deandG; auto; repeat split; unfold g; auto; try red; intros.
...
Proof.
  ...
  { eqext.
    - appA2H H0. destruct H1. appoA2H H1. destruct H2; subst. appA2G.
  ...
  { red; exists \{\ λ u v, u∈x /\ v = [u] \}\.
    ...
    - PP H1 a b. eauto.
    - appoA2H H1. appoA2H H2. appoA2H H3. appoA2H H4. destruct H5, H6.
      subst. apply MKT41; eauto. rewrite <- H8. appA2G.
...
Lemma Lemma163a : ∀ {x y}, Ensemble x -> ~ x ∈ y -> y = (y ∪ [x]) ~ [x].
Proof.
  intros; eqext.
...
Proof.
  ...
  - appA2H H3. destruct H4. appA2H H4.
    assert ((PlusOne k) ∈ W). { apply MKT134; auto. }
    appA2G. split; red; intros.
    + appA2G. apply MKT111 in H0; apply MKT111 in H7; auto. 
    + assert (y ∈ W). { appA2H H7. appA2G. eapply MKT132; eauto. }
...
Fact finse : ∀ f {y u z}, P[y] = PlusOne u -> u ∈ W -> Function f -> 
  Function f⁻¹ -> dom(f) = y -> ran(f) = PlusOne u -> z ∈ y -> P[y ~ [z]] = u.
Proof.
...
Lemma lem167b : ∀ {f r }, W ⊂ ran(f) -> WellOrdered r⁻¹ dom(f) ->
  Order_Pr f r E -> False.
Proof.
  intros. destruct H0.
  set (S:=\{ λ u, ∃ w, w ∈ W /\ [u, w] ∈ f \}).
  assert (S ⊂ dom(f)).
  ...
  - destruct H as [r []]. New (MKT107 MKT113a).
    ...
    + apply MKT114 in H8. New MKT165. 
      ...
      * apply MKT96 in H6 as [[_ ?]]. rewrite reqdi in H12.
        ...
        destruct (MKT160 H6 H14); [eapply H11; eauto|rewrite H16; auto].
...
Lemma lem168 : ∀ {x y r s}, WellOrdered r x -> WellOrdered s y -> 
  WellOrdered \{\ λ u v, (u∈x /\ v∈x /\ Rrelation u r v) \/ (u∈(y ~ x) /\
  v∈(y ~ x) /\ Rrelation u s v) \/ (u∈x /\ v∈(y ~ x)) \}\ (x ∪ y).
Proof.
  intros. repeat split; try red; intros.
  - destruct H. deHun.
...
Proof.
  ...
  - split; try red; intros.
    + destruct (lem168 H H0).  New (H5 _ _ H3 H4). deor; auto. 
      ...
      * left. appoA2G; rxo.
    + TF (\{λ z, z∈y0 /\ z∈(y ~ x)\}=Φ).
      * destruct (lem168 H1 H2) as [_ ?], (H6 _ H3 H4) as [z []]. exists z.
        split; auto. red; intros. appoA2H H10. appoA2H H11. deor; deand.
...
Theorem MKT169 : ∀ x,
  Finite x -> (∀ z, z∈x -> Finite z) -> Finite (∪ x).
Proof.
  intros. assert (\{λ u, u∈W /\ ∀ y, P[y] = u ->
    (∀ z, z∈y -> Finite z) -> Finite (∪ y) \} = W).
  ...
  red in H. rewrite <- H1 in H. appA2H H. destruct H2. apply H3; auto.
Qed.
...
Proof.
  intros. TF (y = Φ).
  { assert ((x × Φ) = Φ). { eqext; try emf. PP H2 a b. destruct H4. emf. }
  ...
  - assert (x ≈ \{λ z, ∃ u, u ∈ x /\ z = [u] × y \}).
    { exists (\{\ λ u v, u ∈ x /\ v = [u] × y \}\).
      ...
      - appA2H H2. destruct H3. appoA2H H3. destruct H4. subst. appA2G.
      - appA2H H2. destruct H3, H3. subst. appA2G. exists x0. appA2G. }
    New (Property_Finite H). New (eqvp H3 (MKT146 H2)).
    unfold Finite in *. apply MKT154 in H2; auto. rewrite <- H2; auto.
  - appA2H H2. destruct H3 as [w []]; subst.
    ...
    { exists (\{\ λ u v, u ∈ y /\ v = [w,u] \}\).
      ...
      - appA2H H4. destruct H5. appoA2H H5. destruct H6. subst. appoA2G.
      - PP H4 a b. destruct H6. eins H5.
        appA2G. exists b. appoA2G. }
    New (Property_Finite H0). New (eqvp H5 (MKT146 H4)).
    unfold Finite in *. apply MKT154 in H4; auto. rewrite <- H4; auto.
Qed.
...
Proof.
    ...
    - appA2H H. deand. New (MKT134 H0). appA2G. split; auto. intros.
      ...
      { exists \{\ λ v w, v ∈ pow(y ~ [f⁻¹[u]]) /\ w = v ∪ [f⁻¹[u]] \}\.
        ...
        - appoA2H H9. appoA2H H10. appoA2H H11. appoA2H H12.
          ...
          + assert (z0 ∈ z∪[f⁻¹[u]]). { rewrite <- H15. appA2G. }
            apply H13 in H17. apply setminp in H17 as []. deHun; tauto. 
          + assert (z0 ∈ y0∪[f⁻¹[u]]). { rewrite H15. appA2G. }
            apply H16 in H17. apply setminp in H17 as []. deHun; tauto.
        - appA2H H9. destruct H10. appoA2H H10. tauto.
        - appA2G. exists (z∪[f⁻¹[u]]). appoA2G. rxo.
          apply AxiomIV; eauto.
        - appA2H H9. destruct H10. appoA2H H10. destruct H11. subst.
          appA2H H11. appA2G; split; try red; intros; [deHun|appA2G].
        ...
        - appA2H H9. deand. subst. appA2G. exists (z ~ [f⁻¹[u]]).
          ...
          red; intros. apply setminp in H12 as []. apply setminP; auto. }
  ...
  red in H. rewrite <- H0 in H. appA2H H. destruct H1. apply H2; auto.
Qed.
...
Theorem MKT173 : ∀ x,
  Ensemble x -> ~ Finite x -> ∃ y, y ⊂ x /\ y <> x /\ x ≈ y.
Proof.
  intros. apply MKT19 in H. rewrite <- MKT152b in H.
  ...
  assert (K1 : ∀ x, x ∈ W -> x ∈ dom(f)).
  { intros. subst. rewrite H6; auto. }
  assert (K2 : ∀ x, x ∈ W -> f[x] ∈ ran(f)).
  { intros. apply Property_dm; auto. }
  exists (x ~ [f[Φ]]). deandG; eauto. eapply MKT147; eauto.
  exists \{\ λ u v, u ∈ P[x] /\ ((u ∈ W -> v = f[PlusOne u])
    /\ ( ~ u ∈ W -> v = f[u])) \}\. rewrite <- H6.
  repeat split; auto; try red; try eqext; intros.
  - appoA2H H9. appoA2H H10. deand. TF (x0 ∈ W).
  ...
  - appA2H H9. destruct H10. appoA2H H10. tauto.
  - appA2G. TF (z ∈ W).
    + exists f[PlusOne z]. appoA2G; rxo. deandG; auto. intros. tauto.
    + New H9. apply Property_dm in H9; auto.
      exists f[z]. appoA2G; rxo. deandG; auto. intro. tauto.
  - appA2H H9. destruct H10. appoA2H H10. deand. TF (x0 ∈ W).
  ...
  - apply setminp in H9 as []. appA2G. subst. einr H9. TF (x ∈ W).
    + destruct (caseint H11) as [?|[n []]]; subst; [elim H10; eauto|].
...
Proof.
  ...
  assert (∃ z, z ∈ x /\ ~ z ∈ y).
  { Absurd. elim H3. apply MKT27; split; auto. red; intros.
    Absurd. elim H5. eauto. } destruct H5 as [z []], H4 as [f [? []]].
  ...
  apply MKT158 in H10. apply MKT158 in H1. apply MKT33 in H2 as ?; eauto.
  apply MKT154 in H9; eauto; [|apply AxiomIV; eauto].
  rewrite <- H9 in H10. destruct H1, H10; auto. destruct (MKT102 _ _ H1 H10).
Qed.

Lemma lem177a : ∀ {a b}, a ∈ R -> b ∈ R -> Max a b = a \/ Max a b = b.
Proof.
...
Lemma lem177c : ∀ P a b c d, Ensemble ([a,b]) -> Ensemble ([c,d]) -> 
  Rrelation ([a, b]) \{\ λ a b, ∃ u v x y, 
  a = [u,v] /\ b = [x,y] /\ P u v x y \}\ ([c, d]) <-> P a b c d.
Proof.
  split; intros.
  - appoA2H H1. destruct H2, H2, H2, H2, H2, H3.
    apply MKT55 in H2 as []; apply MKT55 in H3 as []; ope. subst; auto.
...
Proof.
  ...
  - NEele H0. New H0. apply H in H1. PP H1 a b. destruct H3.
    ...
    assert (s1 <> Φ).
    { apply NEexE. destruct (lem177a H2 H3); [exists a|exists b]; appA2G. }
    ...
    assert (s2 <> Φ).
    { New H7. apply H in H7. appoA2H H7. destruct H10. apply NEexE.
      exists x; appA2G. }
    apply (MKT107 MKT113a) in H9 as [u []]; auto. appA2H H9. rdeHex.
    set (s3 := \{ λ b, ∃ a, [a,b] ∈ y /\ Max a b = Max x x0 /\ a = u \}).
    assert (s3 ⊂ R).
    ...
    + apply (H10 p); [appA2G|appoA2G]; ope. rewrite <- H17, <- H23; eauto.
    + apply (H15 q); [appA2G|appoA2G]. rewrite <- H17, <- H23; eauto.
Qed.

Lemma lem178a : ∀ {a b}, a ∈ R -> b ∈ R -> a ∈ PlusOne (Max a b).
Proof.
  intros. destruct (lem177a H H0); rewrite H1; appA2G.
  apply MKT29 in H1. appA2H H. appA2H H0. destruct (Th110ano H2 H3); auto.
  right. apply MKT41; eauto. apply MKT27; auto.
Qed.
...
Proof.
  ...
  - appoA2H H9. appoA2H H10. destruct H11, H11, H12, H12. deand. subst.
    apply MKT49b in H9 as []. apply MKT55 in H12 as []; subst; ope; auto.
  - PP H9 a b; eauto.
  - appoA2H H9. appoA2H H10. appoA2H H11. appoA2H H12. 
...
Theorem MKT179 : ∀ {x}, x ∈ (C ~ W) -> P[x×x] = x.
Proof.
  intros. Absurd.
  assert (\{ λ x, x ∈ (C ~ W) /\ P[x×x] <> x \} <> Φ).
  { apply NEexE. exists x. appA2G. }
  assert (\{ λ x, x ∈ (C ~ W) /\ P[x×x] <> x \} ⊂ C).
  ...
  apply MKT27; split.
  - assert (ran(f) ⊂ z).
    { red; intros. einr H19. New H19. rewrite H11 in H19. PP H19 u v. deand.
      apply MKT96 in H15 as ?. destruct H24 as [[_]].
      set (d := \{\ λ a b, Rrelation ([a,b]) ≪ ([u,v]) /\ [a,b] ∈ dom(f) \}\).
      assert (d ≈ f[[u,v]]).
      { assert (Function (f|(d))). { apply MKT126a; auto. }
        ...
        - eqext.
          + einr H28. rewrite MKT126c in *; auto. rewrite H27 in H28.
            PP H28 a b. deand. apply H15 in H30; auto. appoA2H H30; auto.
          + apply MKT114 in H17. apply H17 in H28 as ?; auto.
            einr H29. New H29. rewrite H11 in H29. PP H29 a b. deand.
            apply Property_Value' in H30 as ?; auto. appA2G.
            exists [a,b]. appA2G. split; auto.
            appoA2G. split; eauto. appoA2G. split; auto.
      ...
      { apply CsubR in H12. appA2H H12. New H29.
        eapply MKT111 in H29; eauto. assert ((Max u v) ∈ R) by appA2G.
        destruct (pclec H31); [eapply H30|rewrite H32]; eauto. }
      apply MKT33 in H27 as ?; [|apply MKT74; apply AxiomIV; eauto].
      apply MKT154 in H26; eauto. apply MKT158 in H27. rewrite H26 in H27.
      TF (P[Max u v] ∈ W).
      ...
      - assert (Ensemble (Max u v)) by eauto.
        ...
        assert (P[Max u v] ∈ (R ~ W)). { apply setminP; auto. } 
        ...
        + intro. apply H30. New H36. apply MKT164, MKT156 in H36 as [].
          rewrite H38; auto. }
    assert (dom(f) ≈ ran(f)). { apply MKT96 in H15 as []. exists f; auto. }
  ...
  - set (g:= \{\ λ u v, u ∈ z /\ v = [u,u] \}\).
    assert (Function g).
    { split; unfold g; auto. intros. appoA2H H19. appoA2H H20.
      deand. subst; auto. }
     assert (Function1_1 \{\ λ u v, u ∈ z /\ v = [u,u] \}\).
    { split; auto. split; try red; intros.
      - PP H20 a b; eauto.
      - appoA2H H20. appoA2H H21. appoA2H H22. appoA2H H23.
        deand. subst; auto. apply MKT55 in H26 as []; subst; eauto. }
    assert (dom(g)=z).
    { eqext.
...
Fact wh1 : ∀ {x y}, Ensemble x -> y <> Φ -> P[y] ⊂ P[x] -> P[x] ≼ P[y×x].
Proof.
  intros. NEele H0.
   set (f:= \{\ λ u v, u ∈ x /\ v = [x0,u] \}\).
    assert (Function f).
    { split; unfold f; auto. intros. appoA2H H2. appoA2H H3.
      deand. subst; auto. }
     assert (Function1_1 \{\ λ u v, u ∈ x /\ v = [x0,u] \}\).
    { split; auto. split; try red; intros.
      - PP H3 a b; eauto.
      - appoA2H H3. appoA2H H4. appoA2H H5. appoA2H H6.
        deand. subst; auto. apply MKT55 in H9 as []; subst; eauto. }
    assert (dom(f)=x).
    { eqext.
      + appA2H H4. destruct H5. appoA2H H5. tauto.
    ...
    - apply MKT33 in H5; eauto. apply MKT74; auto.
      assert (Ensemble P[y]).
      { apply Property_PClass in H. eapply MKT33; try apply H1; eauto. }
...
Proof.
  ...
  exists (\{\ λ u v, u ∈ x×y /\ v = [Second u, First u] \}\).
  ...
  - eqext.
    + appA2H H1. destruct H2. appoA2H H2. destruct H3. subst.
      PP H3 a b. destruct H5. rewrite MKT54a, MKT54b; eauto. appoA2G. rxo.
...
Theorem MKT180a : ∀ x y,
  x ∈ C -> y ∈ C -> y ∉ W -> x ≠ Φ -> P[x]⊂P[y] -> 
  P[x × y] = Max P[x] P[y].
Proof.
  intros. assert (y ∈ (C ~ W)) by auto. New H4.
...
Theorem MKT180b : ∀ x y,
  x ∈ C -> y ∈ C -> y ∉ W -> x ≠ Φ -> y ≠ Φ -> P[x × y] = Max P[x] P[y].
Proof.
  intros. assert (P[x]⊂P[y] \/ P[y]⊂P[x]).
  { New H. New H0. apply MKT156 in H as []. apply MKT156 in H0 as [].
    rewrite H6, H7. apply CsubR in H4. apply CsubR in H5.
    appA2H H4. appA2H H5. apply MKT109; auto. } destruct H4.
  - apply MKT180a; auto.
...
Theorem MKT181a : ∃ f, Order_Pr f E E /\ dom(f) = R /\ ran(f) = C ~ W.
Proof.
  New (MKT107 MKT113a). assert ((C ~ W) ⊂ R).
  ...
  - eapply MKT91 in H8 as [v []]; eauto.
    ...
    elim MKT162. rewrite <- (wh4 W); [|apply MKT164].
...
Proof.
  ...
  assert (rSection ran(f) E (C ~ W)).
  { rewrite H3. red. deandG; auto. rewrite <- H3; auto. }
  assert (rSection ran(g) E (C ~ W)).
